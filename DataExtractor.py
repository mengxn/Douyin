...
class Extractor:
    ...
    def safe_extract(
            ...
            attribute_chain: str,
            default: str or int or list or dict or SimpleNamespace = ""):
    ...
    def __condition_filter(container: SimpleNamespace):
        result = [i for i in container.all_data]
        container.all_data = result

    def summary_works(self, data: list[dict]):
        print(f"当前账号筛选作品数量: {len(data)}")
    ...
    def _extract_user_data(
        ...
        container.cache["cover"] = self.safe_extract(data, "cover_url[0].url_list[-1]")
        container.cache["short_id"] = self.safe_extract(data, "short_id")
        container.cache["aweme_count"] = str(self.safe_extract(data, "aweme_count", "-1"))
    ...
    def _search_result_classify(
        ...
        #     pass
        print(f"Unreported search results: {data}", False)
    ...
    def _extract_item_records(self, data: list[dict]):
        for i in data:
            print.info(f"{i['type']} {i['id']} 数据提取成功", False)
