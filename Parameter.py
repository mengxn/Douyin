...
class MsToken:
    ...
    @staticmethod
    def get_real_ms_token() -> dict or None:
        if response := send_request(
                MsToken.API,
                MsToken.HEADERS,
                dumps(MsToken.DATA or {"tspFromClient": int(time() * 1000)}),
        ):
...
class WebID:
    ...
    @staticmethod
    def get_web_id(user_agent: str) -> str or None:
